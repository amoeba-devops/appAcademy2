...
export class AmaOidcServiceRef {
  constructor(
    @Inject(AMA_OIDC_SERVICE) public readonly service: AmaOidcService,
  ) {}
}
...
export class AmaAuthController {
  ...
  async callback(
    ...
    if (!entry) {
      res.status(400).json({
        error: { code: 'INVALID_STATE', message: 'state expired or unknown' },
      });
      return;
    }
    ...
    const frontend = String(
      this.config.get('FRONTEND_URL', 'http://localhost:3009'),
    ).replace(/\/$/, '');
  ...
  private callbackUrl(): string {
    ...
    const backend = String(
      this.config.get('BACKEND_URL', 'http://localhost:4009'),
    ).replace(/\/$/, '');
