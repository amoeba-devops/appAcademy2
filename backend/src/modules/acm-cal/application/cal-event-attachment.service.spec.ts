...
describe('CalEventAttachmentService', () => {
  ...
  function build(opts: { configured?: boolean; row?: any; count?: number } = {}) {
    const saved: any[] = [];
    const repo = {
      ...
      save: jest.fn(async (x: any) => {
        const row = { id: `cea-${saved.length + 1}`, createdAt: new Date(), ...x };
        saved.push(row);
        return row;
      }),
    };
  ...
  it('rejects a disallowed MIME type', async () => {
    const { svc } = build();
    await expect(
      svc.upload(ENT, EVT, file({ mimetype: 'application/x-msdownload' }), 'u1'),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('streamDownload returns the object stream', async () => {
    const { svc } = build({
      row: { id: 'cea-1', s3Key: 'k', filename: 'f.pdf', mime: 'application/pdf' },
    });
