...
export class UpdateBodaConfigDto {
  ...
  @IsOptional() @IsString() @IsUrl({ require_protocol: true })
  bodaWebUrl?: string;
  ...
  @ApiPropertyOptional({ description: '기본 roomCode (TPI 1:1 수업 = vendor 발급값)' })
  @IsOptional() @IsString() @MaxLength(30)
  ...
  @IsOptional() @IsString() @MinLength(4) @MaxLength(200)
  authKey?: string;

  @ApiPropertyOptional({ description: '신규 Webhook 공유비밀 (보내면 즉시 저장)' })
  ...
  @ApiPropertyOptional({ minimum: 0, maximum: 120, description: '입장 가능 종료 후 M분' })
  @IsOptional() @IsInt() @Min(0) @Max(120)
  graceAfterMin?: number;

  @ApiPropertyOptional({ minimum: 0, maximum: 60, description: 'reconcile 지연 N분' })
  @IsOptional() @IsInt() @Min(0) @Max(60)
