...
export function StdFilters({ value, onChange, scope }: StdFiltersProps) {
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-2">
      ...
      {scope === "WITHDRAWN" ? (
        <span className="text-sm">{t("status.WITHDRAWN")}</span>
      ) : (
        <select
        ...
        >
          ...
          {!scope && <option value="WITHDRAWN">{t("status.WITHDRAWN")}</option>}
          <option value="ALL">
            {t(scope === "CURRENT" ? "navigation.currentAll" : "filter.all")}
          </option>
        </select>
      )}
