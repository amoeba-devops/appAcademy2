          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelClass}>
                {t("field.parentName", "이름")} *
              </label>
              <input
                placeholder={t("detail.enterValue")}
                {...register(`stdParents.${idx}.parName` as const, {
                  required: true,
                })}
            ...
            <div>
              <label className={labelClass}>
                {t("field.parentRelation", "관계")}
              </label>
              <select
                {...register(`stdParents.${idx}.parRelation` as const)}
                className={inputClass}
              >
                <option value="">—</option>
            ...
            <div>
              <label className={labelClass}>
                {t("field.parentPhone", "전화번호")}
              </label>
              <input
                {...register(`stdParents.${idx}.parPhone` as const)}
                className={inputClass}
              />
            </div>
            <div>
              <label className={labelClass}>
                {t("field.parentEmail", "이메일")}
              </label>
              <input
                placeholder={t("detail.enterValue")}
                type="email"
